...
const ConnectionIcon = ({color, message, onClick}) => {
  return <Button onClick={onClick} as={'a'} className={'flat'} title={message}>
...
export class ConnectionStatusIcon extends Component {
  ...
  render () {
    ...
    return (
      <Modal size={'tiny'} dimmer={'blurring'} style={this.inlineStyle.modal} closeIcon
             trigger={<ConnectionIcon color={color} message={message} />}>
        <Modal.Header><Icon name={'circle'} color={color}/> {'Your connection status'}</Modal.Header>
        <Modal.Content>
          <Modal.Description>
            <p>{message}</p>
            <p>{callsMessage}</p>
            {(connected)? <p>If you want to disconnect from TONE, you can use the following button</p> : ''}
